...
class MESAmodel:
    ...
    template_directory : `str / Path`
        Folder location of the template used in the model
    ...
    model_id: `str`
        Identifier for the model. It is used in combination with is_binary_evolution. Possible
        options are `mesastar`, `mesabinary` or `mesabin2dco`

    custom_run_name : `str`
        Name of the directory where the output of MESA will be stored
    ...
    variables : `dict`
        Dictionary with the name and value of the variables to use in the MESA model. This is
        created with the **kwargs if this is the first time creating the object, but is present in
        the case the loading comes from a database
    ...
    namelists_for_template : `dict`
        Dictionary with the options in each MESA namelist that is shared between different models
    ...
    **kwargs : `dict`
    ...
    def __init__(  # type: ignore
        ...
        **kwargs,
        ...
        # flag to control the type of model between star or binary
        self.is_binary_evolution = is_binary_evolution

        # mesa_model_id sets the id for the type of model
        ...
        if len(variables) == 0:
            ...
            for key, value in kwargs.items():
                ...
                if found_variable_name:
                    ...
                    logger.error(f"could not find `{key}` in MESA parameter list")
    ...
    def set_template_namelists(self) -> None:
        """Create namelists with options that do not change for different MESA models
        ...
        if self.is_binary_evolution:
            ...
            if self.model_id == "mesabinary":
                ...
                logger.critical(
                    f"{self.model_id}: unknown id for creating template of binary namelists"
                )
                sys.exit(1)

        else:

            if self.model_id == "mesastar":
    ...
    def save_namelists_to_file(self, name_id: str = "") -> None:
        ...
        if name_id == "template":
            ...
            if (
                self.model_id == "mesabinary"
                or self.model_id == "mesastar"
                or self.model_id == "mesabin2dco"
            ):
                ...
                logger.critical(
                    f"{self.model_id}: unknown id for saving template of binary namelists"
                )
                sys.exit(1)
